...
<?php
   include("../config/db_config.php");
    ...
    if(isset($_POST['register'])){
        ...
        if (empty($phone)) {
            $phone_err = '<span class="text-info">Phone Number is empty.</span>';
        } elseif ((!preg_match('/^[0-9]*$/', $phone)) && (strlen($phone)!=11)) {
            $phone_err = '<span class="text-info">Phone number cant contain letters or special characters</span>';
        } elseif (strlen($phone)!=11){
            $phone_err = '<span class="text-info">Phone number must be 11 digit number.</span>';
        }
        ...
        if ((empty($user_err)) && (empty($fullname_err)) && (empty($pass1_err)) && (empty($email_err)) && (empty($ref_err)) && (empty($attach_err)) && (empty($date_err)) && (empty($time_err))) {
            // $pass = md5($password);
            // var_dump('here');
            if (empty($address1)){
                        ...
                        <p>Already have an account? <a href="./signin.php" class="signin-text">Sign In</a></p>
                            ...
                            <form action ="signup.php" method="POST" class="input-fields" enctype="multipart/form-data">
                                ...
                                <div class="inputs">
                                    ...
                                    <input type="text" class="border" id="phone" name="phone" value="<?php if (isset($_POST['phone'])) { echo htmlentities($_POST['phone']); } ?>" >
                                    <?php echo $phone_err; ?>
                                </div>
                                <div class="inputs">
