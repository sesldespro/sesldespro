...
$run_ref=mysqli_query($dbcon,$check_ref);
    ...
    $check_lockernum = "SELECT LockerNum FROM `rent_details` WHERE LockerStatus='Available'";
        ...
        if(empty($avail_locker)){
            $numlock_err= '<span class="text-info">Locker number should not be blank and no locker is available at this moment. Please try again later.</span>';
        }

        if ((empty($ref_err)) && (empty($attach_err)) && (empty($date_err)) && (empty($time_err)) && (empty($numlock_err))){
            $updateuser = "UPDATE `user_info` SET  `id_key`='$id_key', `end_notice`='', `duration`='', `subscription_email_sent`='', `end_duration_email_notif`='' WHERE fullname='$g_fullname'";
            if(mysqli_query($dbcon, $updateuser)){
                $insert_sub = "INSERT INTO `user_sub_list`(`user_email`, `user_phone`, `user_sub_plan`, `user_sub_days`, `user_sub_locker`, `user_sub_disc`, `user_sub_pay`, `user_reference`, `user_sub_date`, `user_sub_time`, `user_sub_amount`, `user_upload`, `Sub_Status`) VALUES ('".$g_email."','".$g_phone."','Non-subscriber','7 Days','".$locker_num."','0 %','99php','".$ref."','".$sdate."','".$stime."','99', '".$attach."','pending');";
                if(mysqli_query($dbcon, $insert_sub)){
                    $insert_rent = "INSERT INTO `locker_rental`(`rent_date`, `rent_time`, `fullname`, `user_address`, `phone`, `locker_num`, `locker_otp`, `locker_duration`, `email`) VALUES ('".$currentDate."','".$currentTime."','".$g_fullname."','".$g_address."','".$g_phone."','".$locker_num."','".$id_key."','".$ref."','".$g_email."');";
                    if(mysqli_query($dbcon, $insert_rent)){
                        $update_details = "UPDATE `rent_details` SET  `LockerStatus`='Unavailable', `user_fullname`='$g_fullname', `rent_otp`='$id_key', `rent_start_date`='$currentDate', `rent_start_time`='$currentTime'  WHERE LockerNum='$locker_num'";
                        if(mysqli_query($dbcon, $update_details)){
                            echo '<script type="text/javascript"> alert("Successful! Please wait for admin to confirm your request.") </script>';
...
<head>
    ...
    <link rel="stylesheet" href="../../public/css/css/dashboard.css">
    <link rel="stylesheet" href="../../public/css/css_responsive/board.css">
</head>
<body>
            ...
            <ul class="sidebar-list">
                ...
                <a href="../nonsub/notification.php"><li class="list"><svg width="22" height="26" viewBox="0 0 22 26" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path fill-rule="evenodd" clip-rule="evenodd" d="M10.8462 25C11.7734 25 12.5851 24.4804 13.0047 23.7181C13.3324 23.1226 12.7567 22.5385 12.0769 22.5385H9.61538C8.93565 22.5385 8.35929 23.1224 8.68552 23.7188C9.10236 24.4807 9.91079 25 10.8462 25ZM18.5913 17.9759C18.3604 17.7451 18.2308 17.432 18.2308 17.1056V11.4615C18.2308 7.98009 16.5172 5.02108 13.4943 3.92273C13.0332 3.75518 12.6923 3.33677 12.6923 2.84615C12.6923 1.82462 11.8677 1 10.8462 1C9.82462 1 9 1.82462 9 2.84615C9 3.33669 8.65899 3.75498 8.19767 3.92175C5.1655 5.01791 3.46154 7.96793 3.46154 11.4615V17.1056C3.46154 17.432 3.33187 17.7451 3.10105 17.9759L1.2549 19.822C1.09169 19.9852 1 20.2066 1 20.4374C1 20.9181 1.38964 21.3077 1.87029 21.3077H19.822C20.3027 21.3077 20.6923 20.9181 20.6923 20.4374C20.6923 20.2066 20.6006 19.9852 20.4374 19.822L18.5913 17.9759Z" stroke="#576FB0" stroke-width="1.5"/>
                     ...
                     Notification
                     <?php include("../notification/nonsub-getnotif.php");?></li></a>
    ...
    <div class="dashboard-container">
        <div class="navigation">
            <h1><a href="../nonsub/dashboard.php" class="head-navi">Dashboard ></a> Resubscription</h1>
        </div>
        <div class="payment-container">
            <form action ="resubscribe.php" method="POST" class="input-fields" enctype="multipart/form-data">
                ...
                <div id="howToModal" class="modal">
                    ...
                    <div class="modal-content">
                        ...
                        <div class="how-to-steps">5. Provide the transaction details</div>
                        <div class="how-to-steps">6. Resubscribe and wait for the admin to confirm your subscription.</div>
                    </div>
                </div>
                <div class="inputs">
                ...
                <div class="end-text">
                    <div class="how-to-pay">
                        <p>How to pay? <a href="#" id="howTo" class="signin-text">Click here.</a></p>
                    </div>
                    <div class="buttons">
                        <button class="signup-btn" name='resubscribe' type="submit">Resubscribe</button>
                    </div>             
                </div>
